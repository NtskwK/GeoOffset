...
const addRasterToMap = (files: File[]) => {
  ...
  for (const file of files) {
    ...
    if (file.name.endsWith(".tif") || file.name.endsWith(".tiff")) {
      ...
      console.error(
        `${file.name} can not be recognized as a supported raster file and will be skipped.`,
      );
    }
  }
};
...
const addVectorToMap = (files: File[]) => {
  ...
  if (
    files[0].name.endsWith(".geojson") ||
    files[0].name.endsWith(".json") ||
    files[0].name.endsWith(".kml") ||
    files[0].name.endsWith(".kmz") ||
    files[0].name.endsWith(".gpx") ||
    files[0].name.endsWith(".topojson")
  ) {
    const reader = new FileReader();
  ...
  // shp
    ...
    console.error(
      `${files[0].name} can not be recognized as a supported vector file and will be skipped.`,
    );
  }
};
...
const addEntitiesToMap = (files: File[]) => {
  ...
  for (const file of files) {
    ...
    if (ext === ".gltf" || ext === ".glb") {
      ...
      console.error(
        `${file.name} can not be recognized as a supported 3D model file and will be skipped.`,
      );
      continue;
    }
  }
